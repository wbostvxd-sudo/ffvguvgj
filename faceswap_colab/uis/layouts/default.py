...
from faceswap_colab.uis.components import about, age_modifier_options, background_remover_options, common_options, deep_swapper_options, download, execution, execution_thread_count, expression_restorer_options, face_debugger_options, face_detector, face_editor_options, face_enhancer_options, face_landmarker, face_masker, face_selector, face_swapper_options, frame_colorizer_options, frame_enhancer_options, instant_runner, job_manager, job_runner, lip_syncer_options, memory, output, output_options, preview, preview_options, processors, source, target, temp_frame, terminal, trim_frame, ui_workflow, voice_extractor
...
def run(ui : gradio.Blocks) -> None:
	# En Gradio 5.x, la API cambió - favicon_path ya no existe
	# Usar solo inbrowser, y agregar share para Colab
	try:
		# Intentar con parámetros de Gradio 5.x
		ui.launch(
			inbrowser = state_manager.get_item('open_browser'),
			share = True,  # Para Colab - obtener enlace público
			server_name = '0.0.0.0',
			server_port = 7860
		)
	except TypeError:
		# Si falla, intentar sin parámetros nuevos
		try:
			ui.launch(inbrowser = state_manager.get_item('open_browser'))
		except:
			# Último recurso: lanzar sin parámetros
			ui.launch()
