...
from faceswap_colab import face_landmarker, state_manager, translator
from faceswap_colab.common_helper import calculate_float_step
from faceswap_colab.types import FaceLandmarkerModel, Score
...
def render() -> None:
	...
	FACE_LANDMARKER_MODEL_DROPDOWN = gradio.Dropdown(
		...
		choices = faceswap_colab.choices.face_landmarker_models,
	...
	FACE_LANDMARKER_SCORE_SLIDER = gradio.Slider(
		...
		step = calculate_float_step(faceswap_colab.choices.face_landmarker_score_range),
		minimum = faceswap_colab.choices.face_landmarker_score_range[0],
		maximum = faceswap_colab.choices.face_landmarker_score_range[-1]
	)
