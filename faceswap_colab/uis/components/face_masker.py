...
from faceswap_colab import face_masker, state_manager, translator
...
from faceswap_colab.types import FaceMaskArea, FaceMaskRegion, FaceMaskType, FaceOccluderModel, FaceParserModel
...
def render() -> None:
	...
	with gradio.Row():
		FACE_OCCLUDER_MODEL_DROPDOWN = gradio.Dropdown(
			...
			choices = faceswap_colab.choices.face_occluder_models,
		...
		FACE_PARSER_MODEL_DROPDOWN = gradio.Dropdown(
			...
			choices = faceswap_colab.choices.face_parser_models,
	...
	FACE_MASK_TYPES_CHECKBOX_GROUP = gradio.CheckboxGroup(
		...
		choices = faceswap_colab.choices.face_mask_types,
	...
	FACE_MASK_AREAS_CHECKBOX_GROUP = gradio.CheckboxGroup(
		...
		choices = faceswap_colab.choices.face_mask_areas,
	...
	FACE_MASK_REGIONS_CHECKBOX_GROUP = gradio.CheckboxGroup(
		...
		choices = faceswap_colab.choices.face_mask_regions,
	...
	FACE_MASK_BLUR_SLIDER = gradio.Slider(
		...
		step = calculate_float_step(faceswap_colab.choices.face_mask_blur_range),
		minimum = faceswap_colab.choices.face_mask_blur_range[0],
		maximum = faceswap_colab.choices.face_mask_blur_range[-1],
...
def update_face_mask_types(face_mask_types : List[FaceMaskType]) -> Tuple[gradio.CheckboxGroup, gradio.CheckboxGroup, gradio.CheckboxGroup, gradio.Slider, gradio.Group]:
	face_mask_types = face_mask_types or faceswap_colab.choices.face_mask_types
...
def update_face_mask_areas(face_mask_areas : List[FaceMaskArea]) -> gradio.CheckboxGroup:
	face_mask_areas = face_mask_areas or faceswap_colab.choices.face_mask_areas
...
def update_face_mask_regions(face_mask_regions : List[FaceMaskRegion]) -> gradio.CheckboxGroup:
	face_mask_regions = face_mask_regions or faceswap_colab.choices.face_mask_regions
...
def update_face_mask_padding(face_mask_padding_top : float, face_mask_padding_right : float, face_mask_padding_bottom : float, face_mask_padding_left : float) -> None:
	face_mask_padding_top = sanitize_int_range(int(face_mask_padding_top), faceswap_colab.choices.face_mask_padding_range)
	face_mask_padding_right = sanitize_int_range(int(face_mask_padding_right), faceswap_colab.choices.face_mask_padding_range)
	face_mask_padding_bottom = sanitize_int_range(int(face_mask_padding_bottom), faceswap_colab.choices.face_mask_padding_range)
	face_mask_padding_left = sanitize_int_range(int(face_mask_padding_left), faceswap_colab.choices.face_mask_padding_range)
